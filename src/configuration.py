...
class SyncOptions(BaseModel):
    ...
    Runtime options.
    """
    board_id: Union[str, int] = Field(..., title="Board ID")
    group_id: Optional[str] = Field("topics", title="Group ID")  # default to "topics"
...
class Parameters(BaseModel):
    ...
    def _validate(self) -> "Parameters":
        ...
        cols = [m.monday_column_id for m in self.field_mappings]
    ...
    def mapping_dict(self) -> Dict[str, str]:
        ...
        return {m.source_column: m.monday_column_id for m in self.field_mappings}

    @property
    def monday_columns(self) -> List[str]:
        """Ordered list of monday_column_id to be written."""
        return [m.monday_column_id for m in self.field_mappings]
...
class Configuration(BaseModel):
    ...
    def mappings(self) -> Dict[str, str]:
        return self.parameters.mapping_dict

    @property
    def unique(self) -> Tuple[str, str]:
        return self.parameters.identity_pair
